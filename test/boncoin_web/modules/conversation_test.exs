defmodule BoncoinWeb.ConversationTest do
  ...
  defp assert_sent_answer_messenger_msg(scope, messenger_id, msg) do
    ...
    case msg do
      "" -> assert_called MessengerApi, :send_answer_message, [^messenger_id, _] # Can't count the nb of calls in same test
      msg -> assert_called MessengerApi, :send_answer_message, [^messenger_id, ^msg]
    end
  end
  defp assert_sent_answer_messenger_msg(messenger_id, msg) do
    case msg do
      "" -> assert_called MessengerApi, :send_answer_message, [^messenger_id, _] # Can't count the nb of calls in same test
      msg -> assert_called MessengerApi, :send_answer_message, [^messenger_id, ^msg]
    end
  ...
  describe "Messenger conversation workflow" do
    ...
    test "case 1", %{conn: conn} do
      ...
      # 3) Unknown user selects language and receives question to phone number
        ...
        receive_messenger_msg(conn, messenger_id, messenger_params)
        assert_sent_answer_messenger_msg("link_phone_1", messenger_id, "Now we can speak! Please also type your mobile phone number.")
      # 4) Unknown user types wrong phone number and receives 2nd question for phone number
        ...
        receive_messenger_msg(conn, messenger_id, messenger_params)
        assert_sent_answer_messenger_msg("link_phone_2", messenger_id, "Sorry but we need to identify you. Please type your mobile phone number.")
      ...
      # 7) Unknown user types already used phone number and receives question to phone number
        ...
        receive_messenger_msg(conn, messenger_id, messenger_params)
        assert_sent_answer_messenger_msg("link_phone_1", messenger_id, "Sorry mr_X, this phone number is used by another user. Please unlink it first or contact us.")
      # 8) Unknown user types a free phone number and receives confirmation
        ...
        receive_messenger_msg(conn, messenger_id, messenger_params)
        assert_sent_answer_messenger_msg("no_scope", messenger_id, "Your phone number and Messenger account are now linked.\nPlease visit us on http://localhost:4001/offer/new/09110000002")
      ...
      # 13) Known user wants to change his phone number, types a number used by active user and receives alert msg
        ...
        messenger_params = messenger_params(messenger_id, "09110000001")
        receive_messenger_msg(conn, messenger_id, messenger_params)
        assert_sent_answer_messenger_msg("no_scope", messenger_id, "Sorry mr_X, this phone number is used by another user. Please unlink it first or contact us.")
      ...
      # 15) Known user wants to receive list of his active offers but has no offer yet
        ...
        receive_messenger_msg(conn, messenger_id, messenger_params)
        assert_sent_answer_messenger_msg("no_scope", messenger_id, "You don't have any offer yet. Please create your first offer on http://localhost:4001/offer/new/09110000003")
      # 16) Known user wants to quit messenger but has active offers
        ...
        receive_messenger_msg(conn, messenger_id, messenger_params)
        assert_sent_answer_messenger_msg("no_scope", messenger_id, "Sorry mr_X, we cannot unlink your Messenger account because you still have 1 offers. \n\nFor help please send 0")
      ...
      # 19) Known user wants to quit messenger and confirms it
        ...
        messenger_params = messenger_params(messenger_id, "*999#")
        receive_messenger_msg(conn, messenger_id, messenger_params)
        assert_sent_answer_messenger_msg("quit_bot", messenger_id, "Are you sure you want to remove Messenger link ?. If you are sure please type 1")
        messenger_params = messenger_params(messenger_id, "1")
        receive_messenger_msg(conn, messenger_id, messenger_params)
        assert_sent_answer_messenger_msg(messenger_id, "Your Messenger account has been unlinked.\nHope to see you soon on http://localhost:4001")
